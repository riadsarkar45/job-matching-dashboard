...
const JobDetail = () => {
    const { jobs, skillsFromStorage } = useContext(Skills);
    const { id } = useParams();
    ...
    return (
        <div className='bg-gray-50 w-[60rem] p-4 rounded-md text-gray-800'>
            <div>
                <div className='flex gap-4 items-center justify-between mb-7'>
                    <h2 className='text-[1.2rem]'>{job.company}</h2>
                    <button className='bg-blue-200 p-2 rounded-sm text-gray-800'>Apply</button>
                </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-lg w-[50rem] max-h-[80vh] overflow-hidden">
                        ...
                        <div className="grid grid-cols-5 items-center justify-center gap-2 overflow-y-auto max-h-60">
                            {job?.requiredSkills?.map((skill, i) => (
                                ...
                                >
                                    <h2 onClick={() => handleAddNewSkills(skill)}
                                    >{skill}</h2>
                                </div>
                            ))}
                        </div>
